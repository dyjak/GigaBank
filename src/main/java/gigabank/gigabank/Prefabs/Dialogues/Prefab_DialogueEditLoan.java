...
public class Prefab_DialogueEditLoan {
    Dialog<Void> dialog;
    EntityLoan loan;

    public Prefab_DialogueEditLoan(EntityLoan loan) {
        this.loan = loan;

        dialog = new Dialog<>();
        dialog.setTitle("Edit Loan Data");
        dialog.setHeaderText("Edit Loan Data");
        ...
        TextField titleField = new TextField(loan.getTitle());
        titleField.setPromptText("Title");
        TextField amountField = new TextField(String.valueOf(loan.getAmount()));
        amountField.setPromptText("Amount");
        TextField currencyIdField = new TextField(String.valueOf(loan.getCurrency_id()));
        currencyIdField.setPromptText("Currency ID");
        TextField interestPercentageField = new TextField(String.valueOf(loan.getInterest_percentage()));
        interestPercentageField.setPromptText("Interest Percentage");
        TextArea descriptionArea = new TextArea(loan.getDescription());
        descriptionArea.setPromptText("Description");
        DatePicker createDateField = new DatePicker(convertToLocalDate((Date)loan.getCreate_date()));
        DatePicker deadlineField = new DatePicker(convertToLocalDate((Date)loan.getDeadline()));
        TextField statusField = new TextField(String.valueOf(loan.getStatus()));
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == commitButtonType) {
                ...
                System.out.println("Updating loan with ID " + loan.getLoan_id());

                DB_ProceduralUpdater dbProceduralUpdater = new DB_ProceduralUpdater();
                try {
                    dbProceduralUpdater.loanUpdate(loan.getLoan_id(), title, amount, currencyId, interestPercentage, description, Date.valueOf(createDate), Date.valueOf(deadline), status);
                    showAlert(Alert.AlertType.INFORMATION, "Success", "Loan data successfully updated.");
                } catch (SQLException e) {
                    showAlert(Alert.AlertType.ERROR, "Error", "Failed to update loan: " + e.getMessage());
                }
            }
            return null;
        });
