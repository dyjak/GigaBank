...
public class Prefab_DialogueCreateDeposit {
    Dialog<Void> dialog;

    public Prefab_DialogueCreateDeposit(int userId) {
        dialog = new Dialog<>();
        dialog.setTitle("Create New Deposit");
        dialog.setHeaderText("Create New Deposit");
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == createButtonType) {
                ...
                System.out.println("Creating new deposit: " + title);

                DB_ProceduralCreator dbProceduralCreator = new DB_ProceduralCreator();
                try {
                    dbProceduralCreator.depositCreate(title, amount, currencyId, interestPercentage, description, userId, Date.valueOf(createDate), Date.valueOf(deadline), status);
                    showAlert(Alert.AlertType.INFORMATION, "Success", "Deposit successfully created.");
                } catch (SQLException e) {
                    showAlert(Alert.AlertType.ERROR, "Error", "Failed to create deposit: " + e.getMessage());
                }
            }
            return null;
        });
